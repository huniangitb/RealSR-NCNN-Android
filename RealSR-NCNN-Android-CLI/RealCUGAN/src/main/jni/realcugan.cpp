...
int RealCUGAN::process_cpu(const ncnn::Mat& inimage, ncnn::Mat& outimage) const
{
    ...
    // 消除硬拼接产生的 tile 边界伪影(与 mnnsr/realsr 实现一致)。
        ...
        for (int xi = 0; xi < xtiles; xi++)
        {
            ...
            else
            {
                ...
                // 重叠带权重从边缘 0 线性升到有效区 1, 消除拼接缝。
                {
                    ...
                    // 越界保护: 源矩形裁剪到 out_tile 范围内(重叠带数据缺失时退化为硬拼接)
                    if (srcX < 0) { dstX -= srcX; srcX = 0; }
                    if (srcY < 0) { dstY -= srcY; srcY = 0; }
                    if (srcX + (dstX2 - dstX) > out_tile.w) dstX2 = dstX + (out_tile.w - srcX);
                    if (srcY + (dstY2 - dstY) > out_tile.h) dstY2 = dstY + (out_tile.h - srcY);
                        ...
                        for (int yy = 0; yy < dstY2 - dstY; yy++)
                        {
                            ...
                            for (int xx = 0; xx < dstX2 - dstX; xx++)
                            {
                                ...
                                for (int q = 0; q < 3; q++)
                                {
                                    ...
                                    float val = sp[sx] * 255.f;
                                    // RealCUGAN 4x 残差架构: 模型输出为残差, 需加回上采样输入
                                    // (与 postproc 公式 *255 + inptr*255 一致; 仅在 scale==4 时)
                                    if (scale == 4)
                                    {
                                        const int effRow = (dstY + yy) - out_y0;   // 有效区行
                                        const int effCol = (dstX + xx) - out_x0;   // 有效区列
                                        const int ir = prepadding + effRow / scale;
                                        const int ic = prepadding + effCol / scale;
                                        const float* ip = in_tile.channel(q).row(ir);
                                        val += ip[ic] * 255.f;
                                    }
                                    accum[di + q] += val * wgt;
                                }
    ...
    // 重叠区加权归一化: 各像素加权累积除以总权重, 消除 tile 边界伪影
    {
        ...
        {
            float wsum_i = wsum[i];
            if (wsum_i <= 0.5f) continue;  // 未覆盖区域(兜底硬拼接)保持原值
